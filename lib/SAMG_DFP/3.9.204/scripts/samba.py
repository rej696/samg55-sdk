...
class Samba():
    ...
    def write(self, address, data):
        if self.applet is None:
            self.applet = self.initialize()
        self.applet.write(address, data)

    def read(self, address, data):
        if self.applet is None:
            self.applet = self.initialize()
        self.applet.read(address, data)

    def erase(self, address, size):
        if self.applet is None:
            self.applet = self.initialize()
        self.applet.erase(address, size)
...
class AppletBase(object):
    ...
    def set_hardware_breakpoint(slot, address):
        ...
        rev = dev.Read32(0xe0002000) & 0xF0000000
        if rev == 0: #re-encode address for FPB rev 1
